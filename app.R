...
data <- xmlParse("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22fremont%2C%20ca%22)&format=xml&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys")
dat <- xmlToList(data)
dat <- dat[1]
#set up time
currentTime <- Sys.time() %>% substring(12,16) %>% strsplit(split=':')
currTime <- as.numeric(currentTime[[1]][[1]])+as.numeric(currentTime[[1]][[2]])/60
sunrise <- gsub(":"," ",dat$results$channel$astronomy[1]) %>% strsplit(split=" ")
sunrise <- sunrise[[1]] %>% as.vector()
sunset <- gsub(":"," ",dat$results$channel$astronomy[2]) %>% strsplit(split=" ")
...
#get wind direction into letters
...
#threshold for how moist the water should be, higher value = dryer, rn its at 25%
